...
(define (transitive-closure R)
  (define (helpme R a b)
    (cond ((equal? b 0) R)
          (else (union (compose R a) (helpme R (compose R a) (- b 1)))) ))
  (helpme R R(cardinality R)) )


 ;; (cond ((equal? R '()) '())
   ;;     ((null? (cdr R))(union R (cdr R)))
     ;;   (else make-set (union(helpme (compose R R) R))))
;;(define (helpme element R)
  ;;(cond (equal? (length R) 1) (compose '(element) R)
     ;;   (else append(compose '(element) (car R) (helpme '(element) cdr R))))
  ;;)
