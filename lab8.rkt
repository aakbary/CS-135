...
(define (helpful? fA fB zA zB g)
  (cond ((equal? g fB)
        (cons (list fA fB) '()))
        ((equal? fB zB) (cons (list fA fB) (helpful? fA 0 zA zB g)))
        ((equal? fA 0)
         (cons (list fA fB) (helpful? zA fB zA zB g)))
        ((> (+ fA fB) zB) (cons (list fA fB) (helpful? ( - fA (- zB fB)) zB zA zB g)))
        ((<= (+ fA fB) zB) (cons (list fA fB) (helpful? 0 (+ fA fB) zA zB g))))
 )
