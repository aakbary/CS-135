...
(define (helps B)
(if (equal? B '()) '()
    (append (car B) (helps (cdr B)))))

(define (degree G v)
  (if (equal? G empty) 0
      (if (and (equal? (car(car G)) v) (equal? (car(cdr(car G))) v)) (+ 2 (degree (cdr G) v))
          (if (or (equal? (car (car G)) v) (equal? (car (cdr (car G))) v)) (+ 1 (degree (cdr G) v ))
          (degree (cdr G) v))))
  )

(define (all-even? G)
  (helpsme (get-vertices G) G)
  )

(define (helpsme G I)
  (if (equal? G '()) #t
      (if (equal? (modulo (degree I (car G)) 2) 0) (helpsme (cdr G) I) #f))
  )
...
(define (adjacent G v)
  (if (equal? G empty) '()
      (if (equal? (car(car G)) v) (usort (cons (car (cdr(car G))) (adjacent (cdr G) v)))
      (if (equal? (car (cdr (car G))) v) (usort (cons (car (car G)) (adjacent (cdr G) v)))
...
(define (connected? G)
  (cond ((equal? G '()) #t)
      (else (null? (remove (get-vertices G)(usort (dfs G(caar G)))))))
  )
