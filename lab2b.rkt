...
(define (pig-latin word)
  (append(cdr word)(list (car word))(list 'a 'y)))
...
(define (rotate c)
  (define (rewrite b)
    (list (car b) (car(cdr b)))
     )
  (define (opp d)
    (list (* 1 (car d)) (* -1 (car(cdr d))))
     )
  (define (neg e)
    (list (* -1 (car e)) (* -1 (car(cdr e))))
     )
  (list
   (rewrite c)
   (opp(reverse c))
   (neg c)
   (reverse(opp c)))
   ;(reverse (opp(reverse (opp (rewrite c))))
   ;(rewrite( neg c))
   ;(reverse(rewrite(opp c)))
   )
  
                           ;;(* -1 (car(cdr(reverse c))))))))
        ;;(list (* -1 (car c)) (* -1 (car( cdr c)))) (list (* -1 (car( cdr c))) (car c))))

#| (define (slastlist c)
 |((list (car c) (* -1 (car(cdr c)))
 | )))
 |(define (lastlist c)
 |((list (* -1 (car( cdr c))) (car c))
 | ))
 |(append (anblist c) (slastlist c) (lastlist c))
 |#
...
(define (get-streetname b)
  (cdr(car (car (reverse (cdr(reverse(cdr(cdr(cdr(cdr b))))))))))
  )
